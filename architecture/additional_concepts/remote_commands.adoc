...
== Server Operation
The Kubelet handles remote execution requests from clients. The client opens
a connection to the Kubelet, upgrades the connection to use the SPDY protocol
and, creates streams for `stdin`, `stdout`, and/or `stderr`.
